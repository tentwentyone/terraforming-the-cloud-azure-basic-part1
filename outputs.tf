...
#     # Please note that the az_cli_cmd is not a direct equivalent to the _cmd in the original output. It opens port 22 (SSH) on the VM, which is a prerequisite for SSH access. To actually SSH into the VM, you would need to use an SSH client and the VM's IP address.
#   }
# }
